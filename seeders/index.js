...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Rosalee.Zieme5',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('1VtmypHl9jrCIc7') ) {
      let user = new User({
        'password':'1VtmypHl9jrCIc7',
        'username':'Rosalee.Zieme5',
    ...
    if (!admin || !admin.isPasswordMatch('5y5ou65zIeEn0o4') ) {
      let admin = new User({
        'password':'5y5ou65zIeEn0o4',
        'username':'Jace58',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('1VtmypHl9jrCIc7') && userRole){
    ...
    if (admin && admin.isPasswordMatch('5y5ou65zIeEn0o4') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
